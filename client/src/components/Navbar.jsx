...
const Navbar = () => {  
    ...
    return (    
        ...
        <div>
        <p><span className="text-primary text-5xl px-1 py-0.5">Bingo</span></p>
        </div>
        ...
        <div className="hidden sm:flex items-center gap-8">
            <NavLink to="/" className="text-sm text-gray-500 hover:text-gray-800 transition">Home</NavLink>
            <NavLink to="/products" className="text-sm text-gray-500 hover:text-gray-800 transition">All Product</NavLink>
            <NavLink to="/contact" className="text-sm text-gray-500 hover:text-gray-800 transition">Contact</NavLink>
            ...
            <div onClick={() => navigate("/cart")} className="relative cursor-pointer">
                <img src={assets.nav_cart_icon} alt="cart" className="w- h-6 opacity-80" />
                <button className="absolute -top-2 -right-3 text-xs text-white bg-primary w-[18px] h-[18px] rounded-full">{getCartCount()}</button>
            </div>

            {!user ? (<button onClick={()=>{setShowUserLogin(true),setOpen(false)}} 
                className="cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull transition text-white rounded-full">
                Login 
            </button>)
            :(
                <div className="relative group">
                    <img src={assets.profile_icon} alt="profile" className="w-8 h-8 rounded-full coursor-pointer" />
